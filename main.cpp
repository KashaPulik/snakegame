...
public:
    Block() : block(){};
    Block(float size, sf::Vector2f position)
        : size(size), block(sf::Vector2f(size, size)), direction(0)
    {
...
public:
          ...
          field_position(field_position)
    {
        ...
        if (field_pixel_size - (start_position.x - field_position.x)
            < block_size * 3) {
            snake.push_back(Block(
                    block_size,
                    sf::Vector2f(
                            start_position.x - block_size, start_position.y)));
            snake.push_back(
                    Block(block_size,
                          sf::Vector2f(
                                  start_position.x - 2 * block_size,
                                  start_position.y)));
    ...
    void change_direction(int new_direction)
    {
        ...
        if (new_direction + 2 == old_direction
            || new_direction - 2 == old_direction)
            return;
    ...
    void update_food()
    {
        ...
        new_food_position.x = new_food_position.x
                + block_size * (rand() % (field_block_size - 1));
        new_food_position.y = new_food_position.y
                + block_size * (rand() % (field_block_size - 1));
    ...
    void grow()
    {
        ...
        case UP:
            snake.push_back(Block(
                    block_size,
                    sf::Vector2f(
                            tail_position.x, tail_position.y + block_size)));
        ...
        case RIGHT:
            snake.push_back(Block(
                    block_size,
                    sf::Vector2f(
                            tail_position.x - block_size, tail_position.y)));
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                if (is_changed_direction == 0) {
                    if (event.key.code == sf::Keyboard::Up) {
                        snake->change_direction(UP);
                        is_changed_direction = 1;
                    }
